...
* Last night you could only do {tasks} tasks...

* You felt frustrated and considered staying up late

* But you decided to put your faith into the next day

$ And you went to bed, so lack of sleep did not hinder your next day.

* You mustn't squander this gift from your past self.
...
func _ready() -> void:
	...
	last_task = SaveSystem.load_game()[2]
	next_dialogue()
...
func fade_out() -> void:
	...
	await twe.finished
	get_tree().change_scene_to_file("res://rooms/rm_arcade_transition/arcade_transition2.tscn")
...
func next_dialogue() -> void:
	...
	while (!(master_string[i] == '\n' and master_string[i+1] == '\n')) and (i < len(master_string)):
		...
		i += 1
	pos = i + 2
	current_dialogue = current_dialogue.replace("{tasks}", str(last_task))
